...
public class InferenceMap {
    ...
    public static PossibleConditions getPossibleConditionsFor(DataInterpreter.WifiGrade wifiGrade) {
        ...
        if (DataInterpreter.isGoodOrExcellent(allWifiMetrics) &&
                ConnectivityTester.isOnline(wifiGrade.wifiConnectivityMode) &&
                !WifiStateData.isWifiInAnyProblemMode(wifiGrade.wifiProblemMode)) {
            conditions.exclude(WIFI_CONDITIONS);
            return conditions;
        }

        //wifi Connectivity mode -- strong indicators return from these.
        if (ConnectivityTester.isCaptive(wifiGrade.wifiConnectivityMode)) {
            ...
            conditions.exclude(WIFI_CONDITIONS);
            return conditions;
        } else if (ConnectivityTester.isOff(wifiGrade.wifiConnectivityMode)) {
            ...
            conditions.exclude(ROUTER_CONDITIONS);
            return conditions;
        } else if (ConnectivityTester.isDisconnected(wifiGrade.wifiConnectivityMode) || !WifiStateData.isWifiInAnyProblemMode(wifiGrade.wifiProblemMode)) {
            conditions.exclude(ISP_CONDITIONS);
            conditions.exclude(DNS_CONDITIONS);
            conditions.exclude(WIFI_CONDITIONS);
        ...
        if (ConnectivityTester.isOffline(wifiGrade.wifiConnectivityMode)) {
        ...
        if (DataInterpreter.isAbysmalOrNonFunctional(wifiGrade.primaryApSignalMetric)) {
            ...
            if (wifiGrade.wifiProblemMode == WifiStateData.WifiProblemMode.FREQUENT_DISCONNECTIONS) {
