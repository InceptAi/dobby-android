...
import com.inceptai.wifiexpert.expertSystem.StructuredUserResponse;
import com.inceptai.wifiexpert.expertSystem.nlu.ApiAiClient;
...
public class DobbyAi implements ApiAiClient.ResultListener,
    ...
    private List<Integer> getPotentialUserResponses(@Action.ActionType int lastActionShownToUser) {
        ...
        switch (lastActionShownToUser) {
            case ACTION_TYPE_ASK_FOR_LONG_SUGGESTION:
                responseList.add(StructuredUserResponse.ResponseType.YES);
                responseList.add(StructuredUserResponse.ResponseType.NO);
                break;
            case ACTION_TYPE_BANDWIDTH_TEST:
            case ACTION_TYPE_RUN_TESTS_FOR_EXPERT:
                responseList.add(StructuredUserResponse.ResponseType.CANCEL);
                break;
            case ACTION_TYPE_WIFI_CHECK:
                responseList.add(StructuredUserResponse.ResponseType.RUN_BW_TESTS);
                break;
            case ACTION_TYPE_DIAGNOSE_SLOW_INTERNET:
            case ACTION_TYPE_BANDWIDTH_PING_WIFI_TESTS:
                responseList.add(StructuredUserResponse.ResponseType.CANCEL);
            ...
            case ACTION_TYPE_SHOW_SHORT_SUGGESTION:
                break;
            case ACTION_TYPE_LIST_DOBBY_FUNCTIONS:
                responseList.add(StructuredUserResponse.ResponseType.RUN_ALL_DIAGNOSTICS);
                responseList.add(StructuredUserResponse.ResponseType.RUN_BW_TESTS);
                responseList.add(StructuredUserResponse.ResponseType.RUN_WIFI_TESTS);
                break;
            case ACTION_TYPE_ASK_FOR_BW_TESTS:
                responseList.add(StructuredUserResponse.ResponseType.YES);
                responseList.add(StructuredUserResponse.ResponseType.NO);
            ...
            case ACTION_TYPE_ASK_FOR_FEEDBACK:
                responseList.add(StructuredUserResponse.ResponseType.YES);
                responseList.add(StructuredUserResponse.ResponseType.NO);
                responseList.add(StructuredUserResponse.ResponseType.NO_COMMENTS);
                break;
...
//                responseList.add(StructuredUserResponse.ResponseType.RUN_WIFI_TESTS);
        ...
        if (lastSuggestion != null &&
            ...
            responseList.add(StructuredUserResponse.ResponseType.SHOW_LAST_SUGGESTION_DETAILS);
        }

        if (isShowContactHumanButton()) {
            if (!responseList.contains(StructuredUserResponse.ResponseType.CANCEL) &&
                    !userAskedForHumanExpert &&
                    !chatInExpertMode &&
                    lastAction != ACTION_TYPE_CONTACT_HUMAN_EXPERT) {
                responseList.add(StructuredUserResponse.ResponseType.CONTACT_HUMAN_EXPERT);
            }
            //Special case since, chatInExpertMode is not properly yet
            if (!responseList.contains(StructuredUserResponse.ResponseType.CONTACT_HUMAN_EXPERT) && lastActionShownToUser == ACTION_TYPE_SET_CHAT_TO_BOT_MODE) {
                responseList.add(StructuredUserResponse.ResponseType.CONTACT_HUMAN_EXPERT);
            }
        }
...
//                if (responseType == StructuredUserResponse.ResponseType.RUN_ALL_DIAGNOSTICS || responseType == StructuredUserResponse.ResponseType.RUN_WIFI_TESTS) {
//                    iter.remove();
//                }
//            }
//        }
